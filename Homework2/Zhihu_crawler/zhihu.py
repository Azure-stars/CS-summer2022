...
class ZhihuCrawler:
    def __init__(self):
        # store_path = Path.cwd()/"Zhihu_crawler"/"zhihu.json"
...
"""
    ...
    def begin_crawl(self, begin_time) -> Union[int, float]:
        """
...
"""
    ...
    def get_board(self) -> list:
        ...
        model_url = 'https://www.zhihu.com/question'
        board = []
        try:    
            repo = requests.get("https://www.zhihu.com/hot",headers = self.settings['headers'])
        except:
            logger.error("Exception @ fail to get questions board")
            raise
        soup = BS(repo.text,'lxml')
        news = soup.find_all("div", class_ = "HotItem-content")
        for new in news:
            url = new.a['href']
            if(url.startswith(model_url) == False):
                continue
            title = new.find(class_ = 'HotItem-title').text
            heat = new.find(class_ = "HotItem-metrics").text
            heat_len = len(heat)
            excerpt = new.find(class_ = 'HotItem-excerpt')
            if(excerpt != None):
                excerpt = excerpt.text
            heat = heat[0:heat_len - 3]
            qid_pre = url.split('/')
            qid = int(qid_pre[len(qid_pre) - 1])
            board.append(dict(title = title, heat = heat, excerpt = excerpt, url = url, qid = qid))
        return board
    ...
    def get_question(self, qid: int) -> dict:
        ...
        q_detail = {}
        try:
            repo = requests.get('https://www.zhihu.com/question/' + str(qid),headers = self.settings["headers"])
            soup = BS(repo.text,'lxml')
            data = json.loads(soup.find("script", id = 'js-initialData').text)['initialState']['entities']['questions'][str(qid)]
        except:
            logger.error("Exception @ fail to get details of question" + str(qid))
            raise
        q_detail["created"] = int(data['created'])
        q_detail["followerCount"] = int(data['followerCount'])
        q_detail["visitCount"] = int(data['visitCount'])
        q_detail["answerCount"] = int(data['answerCount'])
        q_detail["title"] = data['title']
        q_detail["raw"] = data['detail']
        q_detail["hit_at"] = hit_at = time.time() 
        return q_detail
